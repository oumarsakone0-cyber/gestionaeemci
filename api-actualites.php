...
switch ($action) {
    ...
    case 'create':
        ...
        try {
            ...
            $result = $stmt->execute([
                $input['titre'],
                $input['type'],
                $input['date_debut'] ?? date('Y-m-d'), // Date actuelle par défaut
                $input['date_fin'] ?? null,
    ...
    case 'delete':
        ...
        error_log("Suppression demandée pour ID: " . $id);
        ...
        try {
            // Vérifier d'abord si l'actualité existe
            $checkStmt = $pdo->prepare("SELECT id FROM aeemciste_actualites WHERE id = ?");
            $checkStmt->execute([$id]);
            $exists = $checkStmt->fetch();
            
            if (!$exists) {
                echo json_encode(['success' => false, 'message' => 'Actualité non trouvée (ID: ' . $id . ')']);
                break;
            }
